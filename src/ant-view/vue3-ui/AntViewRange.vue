...
async function onConfirm() {
  ...
  try {
    saving.value = true;
    if (props.service?.saveViewRange) {
      const result = await props.service.saveViewRange(payload);
      if (result?.msg) {
    ...
    props.runtime?.notifyError?.(error instanceof Error ? error.message : "保存视图范围失败");
  } finally {
    saving.value = false;
  }
}
