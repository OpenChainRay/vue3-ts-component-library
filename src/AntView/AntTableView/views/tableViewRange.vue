  ...
  <div class="main">
    <div class='flex'>
      ...
      <div class="flex column condition">
        ...
        <div v-if="disabled === false" class="addClass">
    ...
    <div v-if="disabled === false" style="margin-top:20px;text-align:center">
...
export default {
  ...
  methods: {
    ...
    async initData () {
      ...
      const { data } = await getColumnList(this.currentTableCode).catch((error) => { throw new Error(error) })
      ...
      if (data.code && data.code === 200) {
        ...
        if (this.filters) {
          if (typeof this.filters === 'string') {
            tableFilter = JSON.parse(this.filters)
          } else {
            tableFilter = this.filters
          }
        } else {
          tableFilter = {}
        }
        console.log(tableFilter, 'tableFilter')
        if (typeof tableFilter.relationType !== 'number') {
          tableFilter.relationType = 0
        }
        // tableFilter  默认条件
        const tableSearchList = this.conditionalValue(tableFilter.filters)
        // 关联关系
        this.tableRelationType = tableFilter.relationType
        // region 显示数据格式化 回显checked 和顺序
        this.allList = columns
        console.log(this.allList, 'allList')
        this.allList.forEach(item => {
          item.checked = false
          if (item.defaultIsShow) {
            item.checked = true
        ...
        // region 过滤条件
        let allColumns = []
        allColumns = columns.filter(column => {
        ...
        console.log(this.tableSearchList, this.selectColumnList, 'tableSearchList')
        this.$nextTick(() => {
