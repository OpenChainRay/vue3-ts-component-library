<template>
  <div style="display:inline-block">
    ...
    <a-modal title="默认数据范围" :visible="viewRangeVisable" cancel-text="取消" ok-text="确认" :footer='null' :destroy-on-close="true" :mask-closable="false"
      ...
      <div>
        <tableViewRange ref="viewRange" :tableCode='tableCode' :id="id" :isCustomRequest="isCustomRequest" :filters="filters"
          @handleviewRangeViewCancel='handleviewRangeViewCancel' @handleviewRangeViewOk='handleviewRangeViewOk' :disabled="disabled" />
      </div>
    </a-modal>
  </div>
</template>
...
import tableViewRange from '../AntTableView/views/tableViewRange.vue'
export default {
  name: 'AntTableCondition',
  props: {
    tableCode: {
      type: String,
      default: '',
  ...
  methods: {
    ...
    handleviewRangeViewOk (params) {
      ...
      if (this.isCustomRequest === true) {
        this.$emit('customTableCondition', params)
      } else {
