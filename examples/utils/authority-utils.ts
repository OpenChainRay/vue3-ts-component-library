 ...
 * 检查将要进入的页面在路由配置中是否勋仔
 *
 * @param {*} toRoute 将要进入的页面
 * @param {*} routeList 所有路由配置信息
 */
function hasToRoute (toRoute, routeList) {
  const allRouters = []
  const findRouter = (routeList) => {
  ...
  const hasRouterIndex = allRouters.findIndex((item) => toRoute.name == item.name)
  return hasRouterIndex
}

export { filterMenu, hasAuthority, hasToRoute }
